...
Config.DropMsg = "Hileyi kapat ulan! Discord: https://discord.gg/BW5KA78SXW"
Config.ReasonBanned = "Sunucudan banlandın. Lütfen bizimle iletişime geç!."

Config.WhitelistedPlayers = { -- Adminler için Bypass 
    "steam:11000010a123456",
    "license:123456789abcdef",
}

Tabii, işte metnin Türkçeleştirilmiş ve geliştirilen hali:

------------ Algılama -------------
Config.BasicEnable = true -- Anticheat'in düzgün çalışabilmesi için bu ayarın **true** olarak yapılması gerekir.  
Config.AntiExplosionDamage = true -- Bu, oyuncuların patlamalar nedeniyle ölmesini engeller.  
Config.AntiGodMode = true -- GodMode hilelerinin çoğunu tespit eder, kaydeder ve yasaklar.  
Config.AntiRagdoll = true -- Anti-Ragdoll hilelerini tespit eder.  
Config.AntiInvisible = true -- Oyuncunun görünmez olup olmadığını tespit eder.  
Config.AntiRadar = true -- Radar (Minimap) açık mı kontrol eder. Sunucunuzda minimap varsayılan olarak açıksa, bu seçeneği kapatın; aksi takdirde yasaklamalar alabilirsiniz.  
Config.AntiExplosiveBullets = true -- Oyuncuların patlayıcı silahlar kullanıp kullanmadığını tespit eder.  
Config.AntiSpectate = true -- Oyuncunun izleyici moduna geçip geçmediğini tespit eder.  
Config.AntiSpeedHacks = true -- Oyuncuların hız modları kullanmasını engeller.  
Config.AntiThermalVision = true -- Oyuncunun termal görüş kullanıp kullanmadığını tespit eder.  
Config.AntiNightVision = true -- Oyuncunun gece görüşü kullanıp kullanmadığını tespit eder.  
Config.AntiPedChange = true -- Oyuncunun pedini değiştirip değiştirmediğini tespit eder.  
Config.AntiFreeCam = true -- Oyuncunun freecam (serbest kamera) kullanıp kullanmadığını tespit eder.  
Config.AntiMenyoo = true -- Oyuncunun Menyoo inject edip etmediğini tespit eder.  
Config.AntiGiveArmor = true -- Oyuncunun zırh hilesi yapıp yapmadığını tespit eder.  
Config.AntiAimAssist = true -- Oyuncuların Aim-Assist (nişan yardımı) özelliğini aktifleştirmesini engeller (GTA V).  
Config.SuperJump = true -- Oyuncuların Süper Zıplama hilesi yapmasını engeller.  
Config.AntiResourceStartorStop = true -- Hilecilerin yeni kaynakları durdurmasını veya başlatmasını engeller.  
Config.AntiInfiniteStamina = true -- Oyuncuların sonsuz dayanıklılık hilesi kullanıp kullanmadığını tespit eder (BETA).  
Config.AntiBlacklistedTasks = true -- Yasaklanmış görevleri tespit eder.  
Config.AntiBlacklistedAnims = true -- Yasaklanmış animasyonları tespit eder.  
Config.AntiVDM = true -- Araç hızlarını kontrol eder.  
Config.EventsDetect = true -- Olayları tespit eder.  
Config.AntiBlacklistedWords = true -- Yasaklanmış kelimeleri tespit eder.  
Config.AntiCHNG = true -- Cheat Engine'ı engeller.  
Config.AntiSpawnObjects = true -- Obje yaratmayı engeller.  
Config.AntiSpawnVehicles = true -- Araç yaratmayı engeller.  
Config.AntiSpawnPeds = true -- Ped yaratmayı engeller.  
Config.DeleteBrokenCars = true -- Bozuk arabaları siler.  
Config.AntiAimbot = true -- Aimbot hilesine karşı koruma sağlar.  
Config.AntiEntity = false -- Varlık oluşturmayı engeller (kapalı durumda).  
Config.AntiTaze = true -- Oyuncuyu taze (elektrik şoku) yapmaya karşı korur.  
Config.AntiGiveWeaponEvent = true -- Oyuncuya veya pedd'e silah verme olaylarını engeller.  
Config.AntiBlacklistedKey = true -- Yasaklanmış tuşlara karşı koruma sağlar.  
Config.AntiResource = false -- Kaynağa özel beyaz listeyi aktif eder (önerilmez).  
Config.AntiCrash = true -- Çökme olaylarını yasaklar.  
Config.ProtectPoliceEvent = true -- Polis olaylarını korur.  
Config.ProtectAmbulanceEvent = true -- Ambulans olaylarını korur.
